...
\def\sectionentry#1#2#3#4#5{% section number, section title, page
  ...
  \hbox{\def\insertsectionhead{#2}%
    ...
    {%
      \usebeamerfont{section in head/foot}\usebeamercolor[fg]{section in head/foot}% select formatting
      \ifnum\c@section=#1% current section
        {\kern-0.5pt}\hyperlink{Navigation#3}{{\usebeamertemplate{section in head/foot}}}%
      \else% not current section
        {\kern-0.5pt}\hyperlink{Navigation#3}{{\usebeamertemplate{section in head/foot shaded}}}%
      \fi}%
  }%
  \ht\beamer@sectionbox=1.1ex% section box's hyperlink upper bound (match height of text)
  \dp\beamer@sectionbox=0ex% section box's hyperlink lower bound
  \fi\ignorespaces}
...
{%
  \begin{beamercolorbox}{section in head/foot}%
    \begin{minipage}[c][4.764mm]{0.96\paperwidth}%
    \insertnavigation{0.96\paperwidth}%
    \end{minipage}%
    \begin{minipage}[c][4.764mm]{0.0249\paperwidth}%
    \raggedleft%
    \insertframenumber%
    \end{minipage}%
  \end{beamercolorbox}%
...
\makeatletter
\newlength\mycirclesize
\setlength\mycirclesize{0.05cm}

\setbeamertemplate{mini frame}{%
  \beamer@xelatex@fixminiframes%
  \begin{pgfpicture}{0.5\mycirclesize}{0.5\mycirclesize}{1.5\mycirclesize}{1.5\mycirclesize}%
    \pgfpathcircle{\pgfpoint{\mycirclesize}{\mycirclesize}}{\mycirclesize}%
...
\setbeamersize{mini frame size=2.65\mycirclesize, mini frame offset=-0.65\mycirclesize}
\makeatother
...
\begin{textblock*}{\paperwidth}(0.335\paperwidth,-0.19\paperheight)\centering%
\begin{minipage}[c]{0.95\paperwidth}\centering%
\includegraphics[height=4.6in]{{\configpath}figures/csu-background.pdf}
\end{minipage}%
\end{textblock*}%

\begin{textblock*}{\paperwidth}(-0.35\paperwidth,0.10\paperheight)\centering%
\begin{minipage}[l]{0.95\paperwidth}\centering%
\textpdfrender{%
TextRenderingMode = FillStroke,
LineWidth = 0.2pt,
FillColor = csugreen,
StrokeColor = csugold,
}{\scalebox{7.5}{\textbf{\theseccount}}}
\end{minipage}%
\end{textblock*}%

\begin{textblock*}{\paperwidth}(0\paperwidth,0.5\paperheight)\centering%
\begin{minipage}[c]{1\paperwidth}\centering%
\begin{columns}\centering
\column{0.8\textwidth}\centering%
\color{fixedtextcolor}\fontsize{24}{32}\selectfont #1%
\end{columns}
\end{minipage}%
\end{textblock*}%
...
\setbeamertemplate{note page}{%
  {%
    ...
    \ifbeamercolorempty[bg]{note title}{}{%
      \insertvrule{.35\paperheight}{mediumgray}%
  ...
  \ifbeamercolorempty[bg]{note page}{}{%
    \nointerlineskip%
    \insertvrule{.65\paperheight}{lightgray}%
